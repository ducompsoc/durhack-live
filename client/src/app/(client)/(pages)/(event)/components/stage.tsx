...
import { useHackathon, IScheduledEvent } from "@/lib/socket";
...
const LiveEventName = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
  return <div
    className={cn("text-lg font-bold pt-[9px]", className)}
    {...props}
  />
}
      ...
      <Card className="ml-[-9px]" style={{ color: "#1e1e1e" }}>
        <Card className="pl-[18px] text-white">
          ...
          <LiveEventName className="row center">
            <LiveEventIcon><DynamicFaIcon iconClassName={inProgressEvent.icon} /></LiveEventIcon>
  ...
  if (upNext) {
    ...
    upNextEl = (
      <Card>
        <h3>Up next</h3>
        <LiveEventName className="row center">
          <LiveEventIcon><DynamicFaIcon iconClassName={upNext.icon} /></LiveEventIcon>
          <div className="grow basis-0">{upNext.name}</div>
        </LiveEventName>
        <div className="pt-3 text-lg text-secondary row center">
          <LiveEventIcon><DynamicFaIcon iconClassName="far fa-clock" /></LiveEventIcon>
          {startDate && (
  ...
  if (onTheSide) {
    onTheSideEl = (
      <Card>
        <h3>Also live now</h3>
        <LiveEventName className="row center">
          <LiveEventIcon><DynamicFaIcon iconClassName={onTheSide.icon} /></LiveEventIcon>
