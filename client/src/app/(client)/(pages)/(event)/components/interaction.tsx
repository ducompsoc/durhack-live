...
const InteractionContainer = ({
  ...
  return (
    <div className={cn("h-[68px] box-border py-3 px-4 mt-1", className)} {...props}>
      {children}
    </div>
  )
}
...
const LargerText = ({ children, className, ...props }: React.HTMLAttributes<HTMLParagraphElement>): React.ReactNode => {
  return (
    <p className={cn("text-xl pb-0.5", className)} {...props}>
      {children}
    </p>
  )
}
...
const EmphasisedText = ({
  ...
  return (
    <p className={cn("text-xl font-bold", className)} {...props}>
      {children}
    </p>
  )
}
...
export const Interaction = React.memo(() => {
  ...
  if (liveLink.startsWith("#")) {
    return (
      <InteractionContainer className="row center">
        <div className="grow basis-0">
          <LargerText>
            Discuss this event in the <strong>{liveLink}</strong> channel in Discord.
          </LargerText>
        </div>
      </InteractionContainer>
    )
  ...
  return (
    <InteractionContainer className="row center">
      ...
      <div>
        <LinkButton href={liveLink} target="_blank">
          Join Zoom session
        </LinkButton>
      </div>
    </InteractionContainer>
